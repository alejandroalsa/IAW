...
(function () {
    ...
    const browsers = [
      ...
      },
      {
        ...
        Buscar: checkContains('opera')
      },
      {
        name: 'Firefox',
        versionRegexes: [/.*?firefox\/\ ?([0-9]+)\.([0-9]+).*/],
        Buscar: checkContains('firefox')
      },
      {
        name: 'Safari',
        versionRegexes: [
    ...
    const oses = [
      ...
      {
        name: 'Android',
        Buscar: checkContains('android'),
        versionRegexes: [/.*?android\ ?([0-9]+)\.([0-9]+).*/]
      },
      {
        name: 'macOS',
        Buscar: checkContains('mac os x'),
        versionRegexes: [/.*?mac\ os\ x\ ?([0-9]+)_([0-9]+).*/]
      },
      {
        name: 'Linux',
        Buscar: checkContains('linux'),
        versionRegexes: []
      },
      {
        name: 'Solaris',
        Buscar: checkContains('sunos'),
        versionRegexes: []
      },
      {
        name: 'FreeBSD',
        Buscar: checkContains('freebsd'),
        versionRegexes: []
      },
      {
        name: 'ChromeOS',
        Buscar: checkContains('cros'),
    ...
    const getPercentageWidths = (warehouse, table, Tablasize) => {
      return getWidthFrom(warehouse, table, getPercentageWidth, deduced => {
        return deduced.fold(() => {
          return Tablasize.minCellWidth();
        }, cellWidth => {
          return cellWidth / Tablasize.pixelWidth() * 100;
        });
      });
    };
    const getPixelWidths = (warehouse, table, Tablasize) => {
      return getWidthFrom(warehouse, table, getPixelWidth$1, deduced => {
        return deduced.getOrThunk(Tablasize.minCellWidth);
      });
    };
    ...
    const noneSize = table => {
      ...
      const getWidths = (warehouse, Tablasize) => getPixelWidths(warehouse, table, Tablasize);
      return {
        width: getWidth,
        pixelWidth: getWidth,
        getWidths,
    ...
    const percentageSize = table => {
      ...
      const getWidths = (warehouse, Tablasize) => getPercentageWidths(warehouse, table, Tablasize);
      return {
        width: getFloatWidth,
        pixelWidth: getWidth,
        getWidths,
    ...
    const clean = (replica, stats, house, widthDelta) => {
      ...
      const Tablasize = Tablasize.getTablasize(replica);
      Tablasize.adjustTableWidth(widthDelta);
    };
    const getTableWidthDelta = (table, warehouse, Tablasize, stats) => {
      if (stats.minCol === 0 && warehouse.grid.columns === stats.maxCol + 1) {
        return 0;
      }
      const colWidths = getPixelWidths(warehouse, table, Tablasize);
      ...
      const newWidth = selectedColsWidth / allColsWidth * Tablasize.pixelWidth();
      const delta = newWidth - Tablasize.pixelWidth();
      return Tablasize.getCellDelta(delta);
    };
    const extract$1 = (table, selectedSelector) => {
      ...
      const Tablasize = Tablasize.getTablasize(table);
      ...
      const widthDelta = getTableWidthDelta(table, house, Tablasize, replicaStats);
    ...
    const RegistroEvents = (editor, actions) => {
    ...
    const getTablasectionType = (table, fallback) => {
      const warehouse = Warehouse.fromTable(table);
    ...
    const run = (operation, extract, adjustment, postAction, genWrappers) => (table, target, generators, behaviours) => {
      const warehouse = Warehouse.fromTable(table);
      const Tablasection = Optional.from(behaviours === null || behaviours === void 0 ? void 0 : behaviours.section).getOrThunk(Tablasection.fallback);
      const output = extract(warehouse, target).map(info => {
        const model = fromWarehouse(warehouse, generators);
        const result = operation(model, info, eq$1, genWrappers(generators), Tablasection);
      ...
      return output.bind(out => {
        ...
        const Tablasizing = Optional.from(behaviours === null || behaviours === void 0 ? void 0 : behaviours.sizing).getOrThunk(() => Tablasize.getTablasize(table));
        const resizing = Optional.from(behaviours === null || behaviours === void 0 ? void 0 : behaviours.resize).getOrThunk(preserveTable);
        adjustment(table, out.grid, out.info, {
          sizing: Tablasizing,
          resize: resizing,
          section: Tablasection
        });
        postAction(table);
    ...
    const mergeTablas = (startAddress, gridA, gridBRows, generator, comparator, lockedColumns) => {
    ...
    const merge$1 = (startAddress, gridA, gridB, generator, comparator) => {
      ...
      return result.map(diff => {
        ...
        return mergeTablas(validStartAddress, fittedGrid, gridBRows, generator, comparator, newLockedColumnsWithinBounds);
      });
    };
    ...
    const replaceRows = (grid, indexes, section, applyScope, comparator, substitution, Tablasection) => {
      ...
      each$2(indexes, index => {
        newRows[index] = Tablasection.transformRow(rows[index], section);
      });
      const newGrid = [
        ...cols,
        ...newRows
      ...
      return replaceIn(newGrid, targets, comparator, substitution, Tablasection.transformCell, scopeGenerator, shouldReplaceCell);
    };
    ...
    const determine = (input, column, step, Tablasize, resize) => {
      const result = input.slice(0);
      const context = neighbours(input, column);
      const onNone = constant(map$1(result, constant(0)));
      const onOnly = index => Tablasize.singleColumnWidth(result[index], step);
      const onLeft = (index, next) => resize.calcLeftEdgeDeltas(result, index, next, step, Tablasize.minCellWidth(), Tablasize.isRelative);
      const onMiddle = (prev, index, next) => resize.calcMiddleDeltas(result, prev, index, next, step, Tablasize.minCellWidth(), Tablasize.isRelative);
      const onRight = (prev, index) => resize.calcRightEdgeDeltas(result, prev, index, step, Tablasize.minCellWidth(), Tablasize.isRelative);
    ...
    const recalculateAndApply = (warehouse, widths, Tablasize) => {
      const newSizes = recalculate(warehouse, widths);
      each$2(newSizes, cell => {
        Tablasize.setElementWidth(cell.element, cell.width);
      });
    };
    const adjustWidth = (table, delta, index, resizing, Tablasize) => {
      const warehouse = Warehouse.fromTable(table);
      const step = Tablasize.getCellDelta(delta);
      const widths = Tablasize.getWidths(warehouse, Tablasize);
      const isLastColumn = index === warehouse.grid.columns - 1;
      const clampedStep = resizing.clampTableDelta(widths, index, step, Tablasize.minCellWidth(), isLastColumn);
      const deltas = determine(widths, index, clampedStep, Tablasize, resizing);
      const newWidths = map$1(deltas, (dx, i) => dx + widths[i]);
      recalculateAndApply(warehouse, newWidths, Tablasize);
      resizing.resizeTable(Tablasize.adjustTableWidth, clampedStep, isLastColumn);
    };
    ...
    const adjustAndRedistributeWidths$1 = (_table, list, details, Tablasize, resizeBehaviour) => {
      const warehouse = Warehouse.generate(list);
      const sizes = Tablasize.getWidths(warehouse, Tablasize);
      const tablePixelWidth = Tablasize.pixelWidth();
      const {newSizes, delta} = resizeBehaviour.calcRedestributedWidths(sizes, tablePixelWidth, details.pixelDelta, Tablasize.isRelative);
      recalculateAndApply(warehouse, newSizes, Tablasize);
      Tablasize.adjustTableWidth(delta);
    };
    const adjustWidthTo = (_table, list, _info, Tablasize) => {
      const warehouse = Warehouse.generate(list);
      const widths = Tablasize.getWidths(warehouse, Tablasize);
      recalculateAndApply(warehouse, widths, Tablasize);
    };
    ...
    const makeRowsSection = (section, applyScope) => (initialGrid, details, comparator, genWrappers, Tablasection) => {
      const rows = uniqueRows(details);
      const rowIndexes = map$1(rows, detail => detail.row);
      const newGrid = replaceRows(initialGrid, rowIndexes, section, applyScope, comparator, genWrappers.replaceOrInit, Tablasection);
      return bundle(newGrid, details[0].row, details[0].column);
    };
    ...
    const fireTablaselectionChange = (editor, cells, start, finish, otherCells) => {
      editor.dispatch('TablaselectionChange', {
        cells,
        start,
        finish,
        otherCells
    ...
    const fireTablaselectionClear = editor => {
      editor.dispatch('TablaselectionClear');
    };
    ...
    const determineDefaultTablastyles = (editor, defaultStyles) => {
    ...
    const Registro = editor => {
      ...
      RegistroOption('table_clone_elements', { processor: 'string[]' });
      RegistroOption('table_use_colgroups', {
        processor: 'boolean',
        default: true
      });
      RegistroOption('table_header_type', {
        processor: value => {
      ...
      RegistroOption('table_sizing_mode', {
        processor: 'string',
        default: 'auto'
      });
      RegistroOption('table_default_attributes', {
        processor: 'object',
        default: { border: '1' }
      });
      RegistroOption('table_default_styles', {
        processor: 'object',
        default: { 'border-collapse': 'collapse' }
      });
      RegistroOption('table_column_resizing', {
        processor: value => {
          const valid = contains$2([
            'preservetable',
            'resizetable'
      ...
      RegistroOption('table_resize_bars', {
        processor: 'boolean',
        default: true
      });
      RegistroOption('table_style_by_css', {
        processor: 'boolean',
        default: true
      });
    };
    ...
    const getTablasizingMode = option('table_sizing_mode');
    const isTablePercentagesForced = editor => getTablasizingMode(editor) === 'relative';
    const isTablePixelsForced = editor => getTablasizingMode(editor) === 'fixed';
    const isTableResponsiveForced = editor => getTablasizingMode(editor) === 'responsive';
    ...
    const getTableDefaultStyles = editor => {
      ...
      return options.isSet('table_default_styles') ? defaultStyles : determineDefaultTablastyles(editor, defaultStyles);
    };
    const tableUseColumnGroup = option('table_use_colgroups');

    const get$5 = (editor, table) => {
      if (isTablePercentagesForced(editor)) {
        return Tablasize.percentageSize(table);
      } else if (isTablePixelsForced(editor)) {
        return Tablasize.pixelSize(table);
      } else {
        return Tablasize.getTablasize(table);
      }
    };
    ...
    const TableActions = (editor, resizeHandler, cellSelectionHandler) => {
      ...
      const getTablasectionType = table => {
        switch (getTableHeaderType(editor)) {
        case 'section':
          return Tablasection.section();
        case 'sectionCells':
          return Tablasection.sectionCells();
        case 'cells':
          return Tablasection.cells();
        default:
          return Tablasection.getTablasectionType(table, 'section');
        }
      };
      ...
      const execute = (operation, guard, mutate, effect) => (table, target, noEvents = false) => {
        ...
        const behaviours = {
          ...
          section: getTablasectionType(table)
        };
    ...
    const RegistroCommands = (editor, actions) => {
      ...
      each$1({
        mceTablasplitCells: () => actOnSelection(actions.unmergeCells),
        ...
        mceTablasizingMode: (_ui, sizing) => setSizingMode(sizing),
    ...
    const RegistroQueryCommands = (editor, actions) => {
      const isRoot = getIsRoot(editor);
    ...
    const fromPoint = (win, x, y) => {
      ...
      return availableBuscar(doc, x, y).map(rng => SimRange.create(SugarElement.fromDom(rng.startContainer), rng.startOffset, SugarElement.fromDom(rng.endContainer), rng.endOffset));
    };
    ...
    const TableCellSelectionHandler = (editor, resizeHandler) => {
      ...
      const onSelection = (cells, start, finish) => {
        ...
        tableOpt.each(table => {
          ...
          fireTablaselectionChange(editor, cells, start, finish, otherCells);
        });
      };
      const onClear = () => fireTablaselectionClear(editor);
      ...
      editor.on('init', _e => {
        ...
        editor.on('TablaselectorChange', e => external$1(e.start, e.finish));
    ...
    const create = (wire, resizing, lazySizing) => {
      ...
      manager.events.adjustWidth.bind(event => {
        ...
        const Tablasize = lazySizing(table);
        adjustWidth(table, delta, event.column, resizing, Tablasize);
    ...
    const TableResizeHandler = editor => {
      ...
      const afterCornerResize = (table, origin, width) => {
        ...
        if (width !== startW && startRawW !== '') {
          ...
          adjustWidth(table, width - startW, col, resizing, Tablasize);
    ...
    const setupTable = editor => {
      ...
      RegistroCommands(editor, actions);
      RegistroQueryCommands(editor, actions);
      RegistroEvents(editor, actions);
      return {
