...
(function () {
    ...
    const Registro$1 = editor => {
      ...
      RegistroOption('autosave_ask_before_unload', {
        processor: 'boolean',
        default: true
      });
      RegistroOption('autosave_prefix', {
        processor: 'string',
        default: 'tinymce-autosave-{path}{query}{hash}-{id}-'
      });
      RegistroOption('autosave_restore_when_empty', {
        processor: 'boolean',
        default: false
      });
      RegistroOption('autosave_interval', {
        processor: timeProcessor,
        default: '30s'
      });
      RegistroOption('autosave_retention', {
        processor: timeProcessor,
        default: '20m'
      });
    };
    ...
    const getAutoSavePrefix = editor => {
      ...
      return editor.options.get('autosave_prefix').replace(/{path}/g, location.pathname).replace(/{query}/g, location.Buscar).replace(/{hash}/g, location.hash).replace(/{id}/g, editor.id);
    };
