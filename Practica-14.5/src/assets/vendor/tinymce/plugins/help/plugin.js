...
(function () {
    ...
    const Registro$2 = (editor, dialogOpener) => {
...
<p>When a non-tabbed dialog is opened, the first interactive component in the dialog will be focused. Usuarios can navigate between interactive Componentes by pressing tab. This includes any footer buttons. Navigation will cycle back to the first dialog component if tab is pressed while focusing the last component in the dialog. Pressing shift + tab will navigate backwards.</p>

<p>When a tabbed dialog is opened, the first button in the tab menu is focused. Pressing tab will navigate to the first interactive component in that tab, and will cycle through the tab\u2019s Componentes, the footer buttons, then back to the tab button. To switch to another tab, focus the tab button for the current tab, then use the arrow keys to cycle through the tab buttons.</p>`;
    const tab$3 = () => {
      const body = {
        type: 'htmlpanel',
        presets: 'document',
    ...
    const convertText = source => {
      ...
      const updated = map(shortcut, segment => {
        const Buscar = segment.toLowerCase().trim();
        return has(replace, Buscar) ? replace[Buscar] : segment;
      });
    ...
    const shortcuts = [
      ...
      {
        shortcuts: ['Meta + F'],
        action: 'Find (if Buscarreplace plugin activated)'
      },
      {
    ...
    var Plugin = () => {
      global$3.add('help', editor => {
        ...
        Registro(editor, dialogOpener);
        Registro$2(editor, dialogOpener);
