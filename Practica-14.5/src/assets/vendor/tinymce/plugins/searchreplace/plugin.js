...
(function () {
    ...
    const markAllMatches = (editor, currentBuscarState, pattern, inSelection) => {
      const marker = editor.dom.create('span', { 'data-mce-bogus': 1 });
      marker.className = 'mce-match-marker';
      const node = editor.getBody();
      done(editor, currentBuscarState, false);
      if (inSelection) {
    ...
    const moveSelection = (editor, currentBuscarState, forward) => {
      const BuscarState = currentBuscarState.get();
      let testIndex = BuscarState.index;
      const dom = editor.dom;
      if (forward) {
        if (testIndex + 1 === BuscarState.count) {
          testIndex = 0;
        } else {
          testIndex++;
        }
        ...
        if (testIndex - 1 === -1) {
          testIndex = BuscarState.count - 1;
        } else {
          testIndex--;
        }
      }
      dom.removeClass(findSpansByIndex(editor, BuscarState.index), 'mce-match-marker-selected');
    ...
    const escapeBuscarText = (text, wholeWord) => {
      const escapedText = text.replace(/[\-\[\]\/\{\}\(\)\*\+\?\.\\\^\$\|]/g, '\\$&').replace(/\s/g, '[^\\S\\r\\n\\uFEFF]');
      const wordRegex = '(' + escapedText + ')';
      return wholeWord ? `(?:^|\\s|${ punctuation() })` + wordRegex + `(?=$|\\s|${ punctuation() })` : wordRegex;
    };
    const find = (editor, currentBuscarState, text, matchCase, wholeWord, inSelection) => {
      const selection = editor.selection;
      const escapedText = escapeBuscarText(text, wholeWord);
      const isForwardSelection = selection.isForward();
      const pattern = {
        regex: new RegExp(escapedText, matchCase ? 'g' : 'gi'),
        matchIndex: 1
      };
      const count = markAllMatches(editor, currentBuscarState, pattern, inSelection);
      if (global$2.browser.isSafari()) {
        selection.setRng(selection.getRng(), isForwardSelection);
      }
      if (count) {
        const newIndex = moveSelection(editor, currentBuscarState, true);
        currentBuscarState.set({
          index: newIndex,
          count,
          text,
          matchCase,
    ...
    const next = (editor, currentBuscarState) => {
      const index = moveSelection(editor, currentBuscarState, true);
      currentBuscarState.set({
        ...currentBuscarState.get(),
        index
      });
    };
    const prev = (editor, currentBuscarState) => {
      const index = moveSelection(editor, currentBuscarState, false);
      currentBuscarState.set({
        ...currentBuscarState.get(),
        index
      });
    ...
    const replace = (editor, currentBuscarState, text, forward, all) => {
      const BuscarState = currentBuscarState.get();
      const currentIndex = BuscarState.index;
      ...
      for (let i = 0; i < nodes.length; i++) {
        ...
        if (all || matchIndex === BuscarState.index) {
          if (text.length) {
      ...
      currentBuscarState.set({
        ...BuscarState,
        count: all ? 0 : BuscarState.count - 1,
        index: nextIndex
      });
      if (forward) {
        next(editor, currentBuscarState);
      } else {
        prev(editor, currentBuscarState);
      }
      return !all && currentBuscarState.get().count > 0;
    };
    const done = (editor, currentBuscarState, keepEditorSelection) => {
      let startContainer;
      let endContainer;
      const BuscarState = currentBuscarState.get();
      const nodes = global$1.toArray(editor.getBody().getElementsByTagName('span'));
      for (let i = 0; i < nodes.length; i++) {
        const nodeIndex = getElmIndex(nodes[i]);
        if (nodeIndex !== null && nodeIndex.length) {
          if (nodeIndex === BuscarState.index.toString()) {
            if (!startContainer) {
    ...
    const hasNext = (editor, currentBuscarState) => currentBuscarState.get().count > 1;
    const hasPrev = (editor, currentBuscarState) => currentBuscarState.get().count > 1;
    ...
    const open = (editor, currentBuscarState) => {
      ...
      const updateButtonStates = api => {
        api.setEnabled('next', hasNext(editor, currentBuscarState));
        api.setEnabled('prev', hasPrev(editor, currentBuscarState));
      };
      const updateBuscarState = api => {
        const data = api.getData();
        const current = currentBuscarState.get();
        currentBuscarState.set({
          ...current,
      ...
      const doFind = api => {
        const data = api.getData();
        const last = currentBuscarState.get();
        ...
        if (last.text === data.findtext && last.matchCase === data.matchcase && last.wholeWord === data.wholewords) {
          ...
          const count = find(editor, currentBuscarState, data.findtext, data.matchcase, data.wholewords, data.inselection);
          if (count <= 0) {
            notFoundAlert(api);
          }
      ...
      const initialState = currentBuscarState.get();
      const initialData = {
      ...
      const spec = {
        ...
        onChange: (api, details) => {
          if (details.name === 'findtext' && currentBuscarState.get().count > 0) {
            reset(api);
          }
        },
        onAction: (api, details) => {
          ...
          case 'find':
            ...
            break;
          case 'replace':
            if (!replace(editor, currentBuscarState, data.replacetext)) {
              reset(api);
            } else {
              updateButtonStates(api);
            }
            break;
          case 'replaceall':
            replace(editor, currentBuscarState, data.replacetext, true, true);
            reset(api);
            break;
          case 'prev':
            prev(editor, currentBuscarState);
        ...
        onClose: () => {
          editor.focus();
          done(editor, currentBuscarState);
          editor.undoManager.add();
        }
      };
    ...
    const Registro$1 = (editor, currentBuscarState) => {
      editor.addCommand('BuscarReplace', () => {
        open(editor, currentBuscarState);
      });
    };

    const showDialog = (editor, currentBuscarState) => () => {
      open(editor, currentBuscarState);
    };
    const Registro = (editor, currentBuscarState) => {
      editor.ui.registry.addMenuItem('Buscarreplace', {
        text: 'Find and replace...',
        shortcut: 'Meta+F',
        onAction: showDialog(editor, currentBuscarState),
        icon: 'Buscar'
      });
      editor.ui.registry.addButton('Buscarreplace', {
        tooltip: 'Find and replace',
        onAction: showDialog(editor, currentBuscarState),
        icon: 'Buscar'
      });
      editor.shortcuts.add('Meta+F', '', showDialog(editor, currentBuscarState));
    };

    var Plugin = () => {
      global$3.add('Buscarreplace', editor => {
        const currentBuscarState = Cell({
          index: -1,
          count: 0,
          text: '',
          matchCase: false,
        ...
        Registro$1(editor, currentBuscarState);
        Registro(editor, currentBuscarState);
        return get(editor, currentBuscarState);
      });
    };
