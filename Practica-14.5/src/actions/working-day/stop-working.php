<?php
    ...
    require "../conection/db-conection.php";
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        // Definimos una consulta SQL donde obtenemos los centiomos que gana al segundo para poder calcular el total de la remuneración
        $remuneration  = $con->query("SELECT second_cents FROM remuneration WHERE user_id = {$_SESSION['user']['id']}");
        $remuneration->execute();
        $user_second_cents = $remuneration->fetch(PDO::FETCH_ASSOC);
        $user_second_cents_value = $user_second_cents['second_cents'];
        ...
        $total_remuneration = $con->prepare("UPDATE records SET total_remuneration = total_seconds * $user_second_cents_value WHERE id = {$user['user_working']};");
